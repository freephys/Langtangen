...
class CurveVizGnuplot(CurveViz):
    ...
    def init(self):
        if self.x is not None and \
           (self._p['xmin'] is not None or \
            self._p['xmax'] is not None):
            self.g('set xrange[%g:%g]' % (self.x[0], self.x[-1]))
